...
float fbm(vec3 p) {
  ...
  // 3^H4 (IOW - expose this as a tunable)  octaves keeps the motion readable while shaving cost on older GPUs.
  for (int i = 0; i < 4; i++) {
    v += a * noise(p);
    p = p * f + shift;
    a *= 0.5;
